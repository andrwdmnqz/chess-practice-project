...
public final class Table extends Observable {
    ...
    private Table() {
        ...
        this.fischerBoard = Board.createFischersBoard();
        this.defaultBoard = chessBoard;
    ...
    private JMenu createOptionsMenu() {
        ...
        final JMenu newGameSubMenu = new JMenu("New Game");
        newGameSubMenu.setMnemonic(KeyEvent.VK_S);

        final JMenuItem newGameItem = new JMenuItem("Classic Chess", KeyEvent.VK_P);
        newGameItem.addActionListener(e -> undoAllMoves());
        newGameSubMenu.add(newGameItem);

        final JMenuItem fischerGameItem = new JMenuItem("Fischer's Chess", KeyEvent.VK_P);
        fischerGameItem.addActionListener(e -> undoAllMovesFischer());
        newGameSubMenu.add(fischerGameItem);

        optionsMenu.add(newGameSubMenu);
    ...
    private void undoAllMovesFischer() {
        for(int i = Table.get().getMoveLog().size() - 1; i >= 0; i--) {
            final Move lastMove = Table.get().getMoveLog().removeMove(Table.get().getMoveLog().size() - 1);
            this.fischerBoard = this.fischerBoard.currentPlayer().unMakeMove(lastMove).getToBoard();
        }
        this.computerMove = null;
        Table.get().getMoveLog().clear();
        Table.get().getGameHistoryPanel().redo(fischerBoard, Table.get().getMoveLog());
        Table.get().getTakenPiecesPanel().redo(Table.get().getMoveLog());
        Table.get().getBoardPanel().drawBoard(fischerBoard);
        Table.get().getDebugPanel().redo();
    }
